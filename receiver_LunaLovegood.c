//This is some modification of my code that was proposed on IRC by LunaLovegood: https://freenode.logbot.info/lkv373a/20210225
//As the original link will expire (or has expired) i archived it here with her permission.
...
static void parse_373_packet(uint8_t const * const buffer, const uint32_t size_data, struct timespec t)
{
	...
	if(!memcmp(magic, magic_video_80, 4))
	{
		...
		if( length > (size_data - 12) )
		{
			flag_fragmented=true;
			frag_total_size=length;
			length = size_data - 12;
		}
		
		uint32_t crypted_size= length & ~15;
		...
		if( length )
		{	//copy remaining
			memcpy(output_video+crypted_size, buffer+12+crypted_size, length);
			sz_output_video+=length;
		}
	...
	else if(!memcmp(magic, magic_video_00, 4))
	{
		...
		memcpy(output_video+sz_output_video, buffer+12, length);
		sz_output_video+=length;
	}
...
int udp_socket_in( uint16_t port )
{
    ...
    if( !port )
    {
        fprintf( stderr, "port non valide: %u\n", (unsigned)port );
        exit(-1);
    }
    ...
    if( -1 == bind( sockfd, (const struct sockaddr *)&sa, sizeof(sa) ) )
    {
        perror("bind");
        fprintf( stderr, "port: %u\n", (unsigned)port );
        exit(-1);
...
int main(int argc, char *argv[])
{
	...
	// DST Address = 0xefff 0x2a2a = 239.255.42.42 (multicast)
	// DST port = 7777 (udp)
//	sock=socket(AF_PACKET, SOCK_DGRAM, htons(0x0800)); //receive only IPv4
	sock = udp_socket_in( 7777 );
	if(sock<0)
...
//			errx(1, "flag_reserved is set in IP-header");
		...
		memcpy(data_buffer, buf, size_rx);
		size_data=size_rx;
...
#endif
		parse_373_packet(data_buffer, size_data, timespec);
